...
**Dernière mise à jour :** 2026-07-04 (sélection de texte à la souris dans `pdf-ui`, avec copie presse-papiers — dernier item du Sprint 9-10 avant le back-end GPU)
...
**Tests :** 107 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement.
...
12. **Sprint 9-10 restant** — back-end GPU `wgpu` (tessellation `lyon`, atlas de glyphes). C'est le dernier point du sprint, et le plus lourd (changement d'architecture du moteur de rendu, pas un ajout incrémental) : tout le reste du Sprint 9-10 est maintenant fait.

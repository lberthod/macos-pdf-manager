...
**Dernière mise à jour :** 2026-07-12 (#32, poignées de déplacement/redimensionnement d'annotation — Sprint 51, voir sprint.md. `pdf_edit::EditSession::set_annotation_rect` [nouveau] ne modifie que `/Rect` de l'annotation [+ `/QuadPoints` remis à l'échelle affine s'il existe] : le flux `/AP /N` existant n'est jamais régénéré, il reste mappé dessus par `resolve_normal_appearance` [déjà là depuis le Sprint 13-14] — un déplacement translate donc exactement le contenu, un redimensionnement l'étire/le compresse. `pdf_app::Session::set_annotation_rect_on_current_page` l'enveloppe [même patron que les autres méthodes d'édition]. `pdf-ui` dessine 4 poignées de coin sur l'annotation sélectionnée et gère leur glissement [`handle_annotation_drag`] : le rect affiché pendant le glissement est recalculé à chaque frame sans toucher `pdf-edit`, un seul `EditOp`/entrée d'annulation au relâchement. Testé bout en bout côté `pdf-edit`/`pdf-app` [déplacement + `/QuadPoints` suivant + persistance après réouverture] ; côté `pdf-ui`, vérifié par build/clippy/lancement sans crash [pas de vérification interactive en environnement avec affichage depuis cet outil]. **Reste ouvert dans #32 : réglage couleur/opacité après création** [demanderait de reparser/régénérer le contenu de l'`/AP` selon le sous-type, hors périmètre de cette passe]. Juste avant (même session) : dernier trou de câblage `pdf-ui` de l'édition comblé, remplir un champ de formulaire au clic [Sprint 50] — `pdf_edit::EditSession::form_fields` liste les champs texte de `/AcroForm/Fields`, `pdf_app::Session::form_fields_on_current_page`/`set_form_field_value_on_current_page` les enveloppent, `pdf-ui` dessine un contour bleu par champ et ouvre `text_modal` préremplie au clic — testé bout en bout sur `acroform_textfield.pdf`. Voir aussi les mises à jour précédentes : premier câblage `pdf-ui` de l'édition [2026-07-05, voir sprint.md "Note de suivi" après le Sprint 17+], édition de texte 6a/6b [Sprint 17+], manipulation de pages [Sprint 15-16], premier socle d'édition [Sprint 13-14], chrome natif macOS [Sprint 11-12], harnais de comparaison pixel + corpus élargi à 25 fixtures [Sprint 0/3-4/7-8] — sprint.md pour le détail sprint par sprint)

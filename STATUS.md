...
**Dernière mise à jour :** 2026-07-04 (panneau signets/plan : lecture de `/Outlines` dans `pdf-core`, exposé par `pdf-app`, affiché dans `pdf-ui`)
...
10. **Sprint 9-10 restant** — back-end GPU `wgpu` (tessellation `lyon`, atlas de glyphes), scroll continu entre pages, sélection de texte à la souris. Ce sont les trois derniers points, tous plus lourds que ce qui a été fait jusqu'ici (le GPU en particulier est un changement d'architecture du rendu, pas un ajout incrémental).

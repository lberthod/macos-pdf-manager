...
- [x] Fenêtre de visualisation prototype (egui) — fait en avance de phase (normalement Sprint 9-10) : `pdf-ui` est un vrai binaire `eframe`/`egui` qui ouvre un fichier (dialogue natif `rfd`), navigue page à page et zoome (re-rasterisation via `pdf_render::render_page_scaled`). Parle directement à `pdf-core`/`pdf-render` en court-circuitant `pdf-app` (toujours un stub) — à corriger quand la couche applicative portera l'état de session. Validé visuellement par capture d'écran sur un fixture réel.

**Critère de sortie (fin Phase 2) :** rendu pixel-comparé conforme sur le corpus, écart sous le seuil défini par le harnais. **Statut réel : quasiment atteint pour le rendu, avec un premier prototype UI en bonus.** Rendu vectoriel, texte (intégré + substitué système) et images JPEG sont fonctionnels et validés visuellement (6 fixtures). Il manque : CFF/Type1/CID, canal alpha `/SMask`, un harnais de comparaison pixel automatisé, et un corpus de test large pour véritablement clore cette phase.
...
- [x] Zoom (boutons ＋/－/réinitialiser, re-rasterisation) et navigation page suivante/précédente — fait en avance de phase dans le prototype `pdf-ui` (voir Sprint 7-8). **Non fait :** scroll continu, molette/pincement trackpad (seulement des boutons pour l'instant).

...
//!
//! Première tranche, volontairement limitée par rapport à `pdf-render` :
//! - Seuls les chemins **remplis** (`PaintOp::Fill`/`FillStroke`, le
//!   remplissage seulement) et les glyphes (toujours remplis, règle
//!   nonzero) sont tessellés (`lyon::tessellation::FillTessellator`) et
//!   dessinés. **Pas de trait** (`PaintOp::Stroke`/`FillStroke` côté trait).
//! - **Pas d'images**, **pas de clip** (`W`/`W*`), **pas de rotation**
//!   (`/Rotate`) — tout ceci existe déjà côté `pdf-render`/CPU et reste la
//!   référence tant que ce backend n'a pas atteint la parité.
//! - Rendu hors-écran uniquement (pas encore branché sur une fenêtre/surface
//!   `pdf-ui`) : `render_page`/`render_page_scaled` créent un contexte
//!   `wgpu` headless (`Instance::request_adapter` sans surface) à chaque
//!   appel, rastérisent dans une texture, et relisent le résultat en RGBA8 —
//!   suffisant pour valider le pipeline par comparaison avec `pdf-render`,
//!   pas optimisé pour un rendu interactif (recréer le device à chaque page
//!   serait couteux dans une vraie boucle de rendu).
//!
...
pub fn render_page_scaled(
    ...
    ctx.render(display, media_box, width, height)
}
...
/// — une simple mise à l'échelle linéaire, sans inversion d'axe (voir la
/// doc de module).
#[derive(Clone, Copy)]
struct PageToNdc {
    origin_x: f64,
...
impl PageToNdc {
    fn map_to_ndc(&self, x: f64, y: f64) -> [f32; 2] {
        let ndc_x = ((x - self.origin_x) / self.width_pts) * 2.0 - 1.0;
        let ndc_y = ((y - self.origin_y) / self.height_pts) * 2.0 - 1.0;
            ...
            match item {
                DisplayItem::Path {
                    ...
                    if !matches!(paint, PaintOp::Fill | PaintOp::FillStroke) {
                        continue; // trait non géré dans cette première tranche.
                    }

...
| N° | Fonctionnalité | Prio | Moteur | Statut | Preuve |
|---|---|---|---|---|---|
| 43 | Remplissage AcroForm (texte, cases, listes) | S | ● | ◐ | **Champs texte (`/Tx`) fermés** : `EditSession::form_fields` + wrappers `pdf-app` + contour cliquable/modale préremplie dans `pdf-ui`. **Cases à cocher (`/Btn`) fermées au Sprint 52** : `EditSession::checkbox_fields` liste les cases qui sont leur propre widget (`/Rect` direct, exclut boutons-poussoir et boutons radio groupés — voir la doc de la méthode) avec leur état `/AS` courant et le nom de l'état "coché" lu dans `/AP /N` ; `set_checkbox_field_value` bascule `/AS`+`/V` sans régénérer l'apparence (`/AP /N` déjà un dictionnaire d'états dans un vrai PDF, résolu tel quel au rendu par `resolve_normal_appearance`) ; `pdf-ui` dessine un contour + remplissage translucide si coché, un clic bascule directement (pas de modale). Testé bout en bout sur `acroform_checkbox.pdf` [nouveau fixture], y compris une vérification que l'état coché/décoché change effectivement ce qui est rendu. **Toujours pas de boutons radio groupés ni de listes/menus déroulants** (`/Ch`) — ligne ◐, pas ☑. |

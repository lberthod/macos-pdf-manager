...
**Dernière mise à jour :** 2026-07-12 (#43, listes/menus déroulants AcroForm — Sprint 54, voir sprint.md. **Ferme #43 entièrement** (◐ → ☑ dans audit50quest.md) : les 4 sous-types cités par la ligne (texte, cases, boutons radio, listes) ont maintenant un chemin `pdf-edit` → `pdf-app` → `pdf-ui` complet et testé. Nouveau fixture `acroform_choice.pdf` [`reportlab.Canvas.acroForm.choice`, `fieldFlags="combo"`, 3 options] : révèle que `/Opt` [tableau de paires `[texte affiché, code]`] a son `/V`/`/DV` égal au **premier** élément de la paire dans ce que produit reportlab en pratique, pas le second — `choice_fields`/`set_choice_field_value` s'alignent dessus plutôt que sur une lecture théorique de la norme. `pdf_edit::EditSession::set_single_line_text_appearance` [nouveau, privé] extrait de `set_form_field_value` la génération d'apparence texte sur une ligne, réutilisée telle quelle par `set_choice_field_value` — **contrairement aux cases/radio, un champ `/Ch` n'a pas déjà les deux apparences dans `/AP /N`**, il faut la synthétiser comme pour un champ texte. `choice_fields`/`set_choice_field_value` [nouveaux, `pdf-edit`] + wrappers `pdf-app` ; `pdf-ui` ouvre une fenêtre de sélection au clic [`show_choice_field_popup`, une seule zone cliquable par champ contrairement aux cases/radio qui en ont une par option]. Testé bout en bout côté `pdf-edit`/`pdf-app` [bascule d'option, persistance après réouverture, au moins un glyphe rendu par caractère du libellé] ; côté `pdf-ui`, vérifié par build/clippy/lancement sans crash [pas de vérification interactive en environnement avec affichage depuis cet outil]. **Limites documentées restantes de #43** (n'empêchent pas ☑) : champs `MultiSelect` non listés, combo éditable [saisie libre hors `/Opt`] non géré, `option_index` d'un groupe radio suppose le groupe sur une seule page. Juste avant (même session) : #43, boutons radio groupés [Sprint 53] puis cases à cocher [Sprint 52] — voir sprint.md pour le détail. Encore avant : #32, poignées de déplacement/redimensionnement d'annotation [Sprint 51, reste ◐ : couleur/opacité après création toujours manquante]. Encore avant : dernier trou de câblage `pdf-ui` de l'édition comblé, remplir un champ de formulaire au clic [Sprint 50]. Voir aussi les mises à jour précédentes : premier câblage `pdf-ui` de l'édition [2026-07-05, voir sprint.md "Note de suivi" après le Sprint 17+], édition de texte 6a/6b [Sprint 17+], manipulation de pages [Sprint 15-16], premier socle d'édition [Sprint 13-14], chrome natif macOS [Sprint 11-12], harnais de comparaison pixel + corpus élargi [Sprint 0/3-4/7-8] — sprint.md pour le détail sprint par sprint)

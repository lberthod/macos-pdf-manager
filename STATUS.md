...
**Dernière mise à jour :** 2026-07-04 (première tranche du back-end GPU `wgpu` — nouveau crate `pdf-render-gpu`, chemins et glyphes tessellés via `lyon`, rendu hors-écran validé sur fixture réel)
...
| Rendu (GPU, `pdf-render-gpu`, première tranche) | Nouveau crate : pipeline `wgpu` **hors-écran** (`Instance::request_adapter` sans surface), chemins remplis **et** contours de glyphes tessellés via `lyon::tessellation::FillTessellator` (même transform que côté CPU pour les glyphes), un seul draw call par page, lecture RGBA8 via `copy_texture_to_buffer`. L'espace de clip `wgpu` a Y vers le haut comme l'espace page PDF : le mapping page->NDC ne nécessite pas de flip d'axe (contrairement au pixmap CPU), vérifié par test dédié. Validé bout en bout sur un fixture réel (glyphes TrueType tessellés, pas seulement des rectangles synthétiques) | Pas de trait (stroke), pas d'images, pas de clip, pas de rotation ; pas encore branché dans `pdf-ui` (rendu hors-écran seulement, recrée un device `wgpu` à chaque appel — pas adapté à une boucle interactive en l'état) ; `pdf-render` (CPU) reste la référence tant que la parité n'est pas atteinte |
...
12. **Back-end GPU `wgpu`** — première tranche faite (voir §2, `pdf-render-gpu`) : chemins + glyphes tessellés `lyon`, rendu hors-écran validé sur fixture réel. Reste à faire pour la parité avec `pdf-render` : trait, images, clip, rotation, atlas de glyphes (au lieu de re-tesseller à chaque frame), et surtout le branchement dans `pdf-ui` (device `wgpu` persistant, intégration avec `eframe` qui supporte déjà un backend `wgpu` en interne). C'est le seul point qui garde le Sprint 9-10 ouvert.

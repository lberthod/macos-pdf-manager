...
**Dernière mise à jour :** 2026-07-04 (mode défilement continu dans `pdf-ui` : toutes les pages empilées, virtualisé via `egui::ScrollArea::show_rows`)
...
**Tests :** 101 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement.
...
11. **Sprint 9-10 restant** — back-end GPU `wgpu` (tessellation `lyon`, atlas de glyphes) et sélection de texte à la souris. Ce sont les deux derniers points, tous deux plus lourds que ce qui a été fait jusqu'ici (le GPU en particulier est un changement d'architecture du rendu, pas un ajout incrémental).

...
**Dernière mise à jour :** 2026-07-04 (corpus de test élargi à 13 fixtures — rotation, formulaire, chiffrement, CJK, document 60 pages — qui a mis en évidence et corrigé deux bugs réels : `/Rotate` ignoré au rendu, erreur trompeuse sur PDF chiffré)
...
**Tests :** 92 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement.
...
| Texte CJK (police simple, non composite) | Rendu visuel correct quand une police TrueType `glyf` est intégrée (contour résolu par code brut via le `cmap` de la police, vérifié visuellement sur le fixture `cjk_text.pdf`) | `pdf-text`/`pdf-core::font::decode_simple` ne récupèrent **aucun caractère Unicode** pour ces glyphes (`/Encoding` WinAnsi/StandardEncoding ne couvre pas ces codes) : le texte s'affiche correctement mais n'est ni extractible ni cherchable |
...
9. **`/ToUnicode`** — lirait le CMap dédié quand présent, ce qui résoudrait au moins partiellement la limite CJK documentée en §2 (texte non extractible) sans nécessiter le support complet des polices composites `/Type0`.

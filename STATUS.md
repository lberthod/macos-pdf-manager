...
**Dernière mise à jour :** 2026-07-12 (#43, boutons radio groupés AcroForm — Sprint 53, voir sprint.md. Nouveau fixture `acroform_radio.pdf` [`reportlab.Canvas.acroForm.radio`, 2 options `"red"`/`"blue"`] : le champ parent `/FT /Btn` [bit `Ff` `Radio` posé] n'a **pas** de `/Rect` propre, seuls ses `/Kids` [les widgets] en ont un — ce qui confirme empiriquement pourquoi `checkbox_fields` [Sprint 52] les excluait déjà naturellement. `pdf_edit::EditSession::radio_groups` [nouveau] liste ces groupes [complément exact de ce que `checkbox_fields` exclut], une `RadioOptionInfo` par widget-enfant [`/Rect`, `on_state`, `selected` lu depuis l'`/AS` de ce widget précis] ; `set_radio_group_value` fixe l'`/AS` de **chaque** enfant [`/Off` sauf l'option choisie] et le `/V` du parent, en un seul `EditOp`/undo pour tout le groupe. `pdf_app::Session::radio_groups_on_current_page`/`set_radio_group_value_on_current_page` enveloppent [filtrent par `/Annots` de la page courante] ; `pdf-ui` sélectionne au clic, contour + remplissage translucide par option. Testé bout en bout côté `pdf-edit`/`pdf-app` [bascule vers l'autre option, persistance après réouverture, **et vérification qu'un seul widget reste effectivement coché dans le fichier final**] ; côté `pdf-ui`, vérifié par build/clippy/lancement sans crash [pas de vérification interactive en environnement avec affichage depuis cet outil]. **Reste ouvert dans #43 : listes/menus déroulants (`/Ch`)** — c'est désormais le seul sous-cas manquant de la ligne. Juste avant (même session) : #43, cases à cocher AcroForm [Sprint 52] — `pdf_edit::EditSession::checkbox_fields`/`set_checkbox_field_value` basculent `/AS`+`/V` sans régénérer `/AP` [un vrai PDF a déjà les deux apparences, `resolve_normal_appearance` — déjà là depuis le Sprint 13-14 — résout laquelle afficher] ; `pdf-ui` bascule au clic. Encore avant : #32, poignées de déplacement/redimensionnement d'annotation [Sprint 51] — `pdf_edit::EditSession::set_annotation_rect` ne modifie que `/Rect`+`/QuadPoints` [rescale affine], sans régénérer l'`/AP` existant ; `pdf-ui` dessine 4 poignées de coin glissables. **Reste ouvert dans #32 : réglage couleur/opacité après création.** Encore avant : dernier trou de câblage `pdf-ui` de l'édition comblé, remplir un champ de formulaire au clic [Sprint 50]. Voir aussi les mises à jour précédentes : premier câblage `pdf-ui` de l'édition [2026-07-05, voir sprint.md "Note de suivi" après le Sprint 17+], édition de texte 6a/6b [Sprint 17+], manipulation de pages [Sprint 15-16], premier socle d'édition [Sprint 13-14], chrome natif macOS [Sprint 11-12], harnais de comparaison pixel + corpus élargi [Sprint 0/3-4/7-8] — sprint.md pour le détail sprint par sprint)
...
27 fixtures PDF dans [pdf-core/tests/fixtures/](./pdf-core/tests/fixtures/) (voir leur [README](./pdf-core/tests/fixtures/README.md) pour le détail et les scripts de régénération — `acroform_checkbox.pdf`/`acroform_radio.pdf` ajoutés aux Sprints 52-53), plus `sample_image.jpg` (Sprint 15-16 : une petite image JPEG synthétique, pas un PDF, utilisée pour tester `pdf-edit::insert_image_page`) :

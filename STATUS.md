...
**Dernière mise à jour :** 2026-07-12 (Sprint 59 — vérification de signature numérique `/Sig`, voir sprint.md. Nouveau module `pdf_core::signature` : `Document::signature_fields()` liste les champs `/FT /Sig` signés (métadonnées `/Name`/`/Reason`/`/Location`/`/M`) ; `SignatureField::verify(&doc)` vérifie une signature PKCS#7/CMS détachée (`adbe.pkcs7.detached`, le gestionnaire dominant en pratique) — **intégrité seulement, pas confiance** [pas de chaîne de certification jusqu'à une autorité racine, pas de révocation OCSP/CRL, documenté explicitement en tête de module comme un projet séparé]. RSA (PKCS#1 v1.5) + SHA-1/256/384/512 ; ECDSA/Ed25519/digest non listé/module RSA >8192 bits renvoient `UnsupportedAlgorithm` plutôt que d'échouer silencieusement. **Feasibilité vérifiée par un sondage séparé avant le code définitif** : `cms` [seule dépendance pré-version du projet, `0.3.0-pre.2`, aucune alternative stable mature pour CMS/PKCS#7 en Rust pur] tire `der`/`spki` 0.8, incompatible **au niveau des types** avec `rsa`/`pkcs8` [0.7 transitif] — contourné par aller-retour en octets DER bruts plutôt que conversion directe, à 3 points de jonction différents. `/Contents` porte un bourrage à zéro au-delà du DER réel [décodeur de longueur DER manuel pour tronquer avant de parser]. Zéro `.unwrap()`/`.expect()` sur une donnée venant du fichier [`/Contents` est une entrée hostile par défaut]. Nouveau fixture réel `signed_document.pdf` [`pyhanko`, RSA-2048/SHA-256, certificat de test auto-signé — ni `reportlab` ni `pikepdf` ne savent signer]. Testé bout en bout : signature intacte acceptée, **falsifier un octet dans `/ByteRange` après signature bien détecté** [le test qui compte le plus]. Nouvelle commande `pdf-cli verify-signatures` vérifiée manuellement sur 3 scénarios réels. **Non fait : production de signature** [nécessiterait de porter une clé privée, projet séparé], câblage `pdf-ui`. Dernier point tractable du Sprint 23+ — reste l'accessibilité VoiceOver [non engagée, intégration architecturale majeure, non vérifiable sans session graphique]. Juste avant (même session) : Sprint 58 — vrai mot de passe utilisateur pour les PDF chiffrés, `audit50quest.md` #50 suite, voir sprint.md. `pdf_core::crypt::Decryptor::new` prend désormais un mot de passe candidat et le **vérifie** contre `/U` (Algorithme 4/5 ISO 32000-1 §7.6.4.4, ou le sel de validation pour R5/6) avant de renvoyer un contexte utilisable — **corrige un vrai bug**, pas seulement une extension : avant cette passe, un mot de passe incorrect (y compris le vide implicite sur un document qui en exige un réel) produisait silencieusement une clé de fichier erronée et donc du contenu déchiffré corrompu, sans jamais échouer. Nouveau `PdfError::IncorrectPassword` (via `DecryptError`, distingue "gestionnaire non standard" de "mot de passe incorrect") propagé par `Document::open_with_password` → `pdf_edit::EditSession::open_with_password` → `pdf_app::Session::open_with_password` (`SessionOpenError::IncorrectPassword`/`Other`) jusqu'à `pdf-ui`, qui ouvre une fenêtre de saisie (champ masqué, réessai en boucle) quand l'ouverture avec mot de passe vide échoue. Nouveau fixture `encrypted_user_password.pdf` (AES-128/R4, mot de passe réel `"secret123"`) : testé bout en bout côté `pdf-core`/`pdf-app` (bon mot de passe déchiffre, mauvais mot de passe et mot de passe vide tous deux rejetés proprement). **Reste ◐ dans audit50quest.md #50 : VoiceOver/accessibilité** (`NSAccessibility`, aucune trace dans le code) — demanderait une intégration architecturale bien plus large (`pdf-ui` dessine tout lui-même via `egui`/`wgpu`, aucun widget natif accessible par défaut), non engagée. Juste avant (même session) : Sprint 57 (cache police/contours, `Interpret` ÷5,5 sur un document de 1500 pages, `pdf-cli bench` nouveau) et Sprint 56 (fuzzing `cargo-fuzz`, deux débordements arithmétiques réels trouvés et corrigés dans `pdf-core::filters`/`parser`) — voir sprint.md pour le détail de ces deux sprints et de tous les précédents (Sprints 49-55 : onglets multi-documents, #32/#43 fermés entièrement, voir `audit50quest.md`).
...
- **Signatures numériques** : **vérification** faite depuis le Sprint 59 (`pdf_core::signature`, voir §2/§5 — intégrité cryptographique seulement, pas de chaîne de confiance/révocation), **production** toujours absente (nécessiterait de porter une clé privée). (Déchiffrement `/Encrypt` : fait pour mot de passe utilisateur vide **et** réel depuis les Sprints 22/58, voir §2.)

---

## 4. Corpus de test

30 fixtures PDF dans [pdf-core/tests/fixtures/](./pdf-core/tests/fixtures/) (voir leur [README](./pdf-core/tests/fixtures/README.md) pour le détail et les scripts de régénération — `acroform_checkbox.pdf`/`acroform_radio.pdf`/`acroform_choice.pdf` ajoutés aux Sprints 52-54, `encrypted_user_password.pdf` au Sprint 58, `signed_document.pdf` au Sprint 59), plus `sample_image.jpg` (Sprint 15-16 : une petite image JPEG synthétique, pas un PDF, utilisée pour tester `pdf-edit::insert_image_page`) :

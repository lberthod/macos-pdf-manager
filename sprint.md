...
- [~] Back-end GPU `wgpu` (Metal) : tessellation des chemins (`lyon`), atlas de glyphes. **Première tranche faite** — nouveau crate `pdf-render-gpu` : pipeline `wgpu` hors-écran (headless, `Instance::request_adapter` sans surface), tessellation `lyon::tessellation::FillTessellator` pour les chemins remplis **et** les contours de glyphes (même logique que `pdf-render::build_glyph_path`, transform appliqué avant tessellation), un seul draw call par page (tout le géométrie tessellée dans un unique vertex/index buffer, respectant l'ordre d'émission pour le "painter's algorithm"), lecture du résultat en RGBA8 via `copy_texture_to_buffer` (avec retrait du padding `COPY_BYTES_PER_ROW_ALIGNMENT`). Espace de clip `wgpu` ayant Y vers le haut comme l'espace page PDF, le mapping page->NDC ne nécessite **pas** le flip explicite qu'il faut côté `pdf-render`/pixmap — vérifié par un test dédié (contenu placé en haut-gauche de la page atterrit bien en haut-gauche de l'image). Validé bout en bout sur le fixture réel `embedded_truetype_font.pdf` (glyphes tessellés et rendus, pas seulement des rectangles synthétiques). **Non fait (voir doc de module) :** trait (`PaintOp::Stroke`), images, clip (`W`/`W*`), rotation (`/Rotate`), et surtout **pas encore branché dans `pdf-ui`** — rendu hors-écran seulement pour l'instant, recrée un device à chaque appel (pas adapté à une boucle de rendu interactive en l'état). `pdf-render` (CPU/`tiny-skia`) reste le backend de référence tant que la parité fonctionnelle n'est pas atteinte.

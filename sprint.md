...
- [~] Back-end GPU `wgpu` (Metal) : tessellation des chemins (`lyon`), atlas de glyphes. **Deux tranches faites** — nouveau crate `pdf-render-gpu` : pipeline `wgpu` hors-écran (headless, `Instance::request_adapter` sans surface), tessellation `lyon::tessellation::{FillTessellator, StrokeTessellator}` pour les chemins **remplis et tracés** et les contours de glyphes (même logique que `pdf-render::build_glyph_path`, transform appliqué avant tessellation), un seul draw call par page (toute la géométrie tessellée dans un unique vertex/index buffer, respectant l'ordre d'émission pour le "painter's algorithm"), lecture du résultat en RGBA8 via `copy_texture_to_buffer` (avec retrait du padding `COPY_BYTES_PER_ROW_ALIGNMENT`). Espace de clip `wgpu` ayant Y vers le haut comme l'espace page PDF, le mapping page->NDC ne nécessite **pas** le flip explicite qu'il faut côté `pdf-render`/pixmap. **Rotation (`/Rotate`) appliquée** directement dans l'espace NDC (`render_page_rotated`, même structure mathématique que `pdf_render::rotation_matrix` mais dans un carré [-1,1] plutôt qu'en pixels — pas besoin des dimensions en points). Les deux dérivations géométriques (orientation Y, rotation 90°) ont été vérifiées par des tests dédiés *avant* de leur faire confiance, et les deux ont été correctes du premier coup. Validé bout en bout sur le fixture réel `embedded_truetype_font.pdf`. **Non fait (voir doc de module) :** images, clip (`W`/`W*`), atlas de glyphes (re-tessellation à chaque appel), et surtout **pas encore branché dans `pdf-ui`** — rendu hors-écran seulement pour l'instant, recrée un device à chaque appel (pas adapté à une boucle de rendu interactive en l'état). `pdf-render` (CPU/`tiny-skia`) reste le backend de référence tant que la parité fonctionnelle n'est pas atteinte.

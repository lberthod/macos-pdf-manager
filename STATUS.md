...
**Dernière mise à jour :** 2026-07-12 (Sprint 57 — optimisation performance sur gros fichiers, voir sprint.md. Deuxième point du Sprint 23+ [durcissement] engagé, même méthode que le fuzzing au Sprint 56 : mesurer avant de deviner. Nouvelle commande permanente `pdf-cli bench <file.pdf>` [temps par étape — ouverture, contenu par page, interprétation, extraction de texte, rendu CPU — sur tout le document, pas une seule page]. Mesuré sur un document synthétique de 1500 pages [texte dense, ~4000 caractères/page, Helvetica non intégrée — pas commité, hors du principe "diversité pas volume" du corpus de fixtures] : `Interpret` ressortait à 3,09 s [2,06 ms/page]. **Bug de performance trouvé, pas une simple optimisation** : `Interpreter::show_text` rechargeait le dictionnaire de police complet à **chaque** `Tj`/`TJ` — comportement documenté comme un compromis délibéré depuis les Sprints 5-6/7-8 ("acceptable tant que les documents restent de taille modeste"). Corrigé par `Interpreter::font_cache` [`BTreeMap<ObjRef, Rc<Font>>`, nouveau champ] : une police référencée indirectement n'est chargée qu'une fois par document. **Deuxième gaspillage trouvé en creusant le premier** : `Font::glyph_outline`/`cid_glyph_outline` reparsaient la police entière (`ttf_parser::Face::parse`) à chaque caractère, alors qu'un texte de milliers de caractères n'a qu'une poignée de codes distincts — corrigé par `outline_cache`/`cid_outline_cache` [nouveaux champs `Font`, `RefCell<HashMap<...>>`]. Résultat mesuré [même document, `pdf-cli bench`] : `Interpret` 3,09 s → 564 ms [÷5,5] ; total 18,01 s → 12,15 s [÷1,5]. `Render CPU` reste dominant [~95 % du total] — rasterisation `tiny-skia` elle-même, pas un gaspillage algorithmique, documenté comme hors périmètre de cette passe [demanderait un atlas de glyphes façon `pdf-render-gpu`, ou une réorganisation plus profonde du pipeline CPU]. Régression : `pdf-core/src/interp.rs::lookup_font_caches_by_object_reference` [`Rc::ptr_eq` entre deux appels consécutifs sur la même référence d'objet]. Juste avant (même session) : Sprint 56 — fuzzing du parser, `cargo-fuzz`, voir [fuzz/README.md](./fuzz/README.md). Premier point du Sprint 23+ [durcissement] engagé : deux cibles libFuzzer, `parse_document` [`Document::open` + balayage superficiel, sans rendu] et `render_document` [comme la première, mais rend la première page en CPU via `pdf-render` — couvre en plus interpréteur de contenu, résolution de polices, décodage d'image]. Nécessite un toolchain **nightly** [`-Zsanitizer=address`, indisponible sur stable] — utilisé uniquement pour cette passe, le reste du projet reste sur stable. **Deux bugs réels trouvés dans les deux premières minutes de fuzzing**, tous deux des débordements arithmétiques [détectés par les assertions de débordement, actives sous `cargo fuzz` **et** sous n'importe quel build `dev` standard de ce workspace] : `pdf-core::filters::ascii85_decode` paniquait sur un octet hors de la plage ASCII85 valide [`b - 33` avec `b < 33`] — corrigé en renvoyant une erreur propre, comme `ascii_hex_decode` le fait déjà pour un chiffre hexadécimal invalide ; `pdf-core::parser::parse_stream_body` paniquait sur un `/Length` négatif [`*n as usize` enroulé, puis `pos + len` débordant] — corrigé en traitant un `/Length` négatif comme absent [repli sur la recherche littérale de `endstream`] et en utilisant `saturating_add` en défense en profondeur. Une régression par bug (`pdf-core/src/filters.rs`/`pdf-core/src/parser.rs`), pas les octets de plantage bruts commités [corpus/artefacts `cargo-fuzz` volontairement exclus de git, voir `fuzz/README.md` pour les reconstituer]. ~260 000 exécutions cumulées après ces deux correctifs, sans nouveau plantage. Juste avant (même session) : #32, couleur/opacité d'annotation — Sprint 55, voir sprint.md. **Ferme #32 entièrement** (◐ → ☑ dans audit50quest.md, couverture Must 81 % → 83 %), sauf `/FreeText` [limite documentée]. `pdf_edit::EditSession::set_annotation_style` [nouveau] régénère entièrement l'apparence (`/AP /N`) selon le `/Subtype`, en conservant la géométrie (`/Rect`, jamais touché) : `/Highlight` [rectangle rempli] via `/ca` [opacité de remplissage] sur un nouvel `ExtGState`, `/Underline`/`/StrikeOut` [ligne tracée] via `/CA` [opacité de **trait**, ces deux types dessinant une ligne, pas un aplat — jusqu'ici sans aucune notion d'opacité]. Refuse explicitement `/FreeText` [son apparence encode le texte affiché lui-même, la régénérer sans le perdre demanderait de le relire depuis `/AP`, hors périmètre]. Testé bout en bout **directement sur le rendu** [`fill_color`/`fill_alpha` pour `/Highlight`, `stroke_color`/`stroke_alpha` pour `/Underline`], pas seulement sur `/C`/`/AP`. `pdf_app::Session::set_annotation_style_on_current_page` + `AnnotationInfo` étendu d'un champ `color` [lu depuis `/C`, préremplit la fenêtre côté UI]. `pdf-ui` : bouton "🎨 Style…" [actif seulement pour les sous-types pris en charge] ouvre une fenêtre avec sélecteur de couleur + curseur d'opacité, appliqués immédiatement à chaque changement. Vérifié côté `pdf-ui` par build/clippy/lancement sans crash [pas de vérification interactive en environnement avec affichage depuis cet outil]. Juste avant (même session) : #43, listes/menus déroulants AcroForm [Sprint 54] — **ferme #43 entièrement** aussi (les 4 sous-types : texte, cases, boutons radio, listes), boutons radio groupés [Sprint 53], cases à cocher [Sprint 52] — voir sprint.md pour le détail. Encore avant : #32, poignées de déplacement/redimensionnement d'annotation [Sprint 51]. Encore avant : dernier trou de câblage `pdf-ui` de l'édition comblé, remplir un champ de formulaire au clic [Sprint 50]. Voir aussi les mises à jour précédentes : premier câblage `pdf-ui` de l'édition [2026-07-05, voir sprint.md "Note de suivi" après le Sprint 17+], édition de texte 6a/6b [Sprint 17+], manipulation de pages [Sprint 15-16], premier socle d'édition [Sprint 13-14], chrome natif macOS [Sprint 11-12], harnais de comparaison pixel + corpus élargi [Sprint 0/3-4/7-8] — sprint.md pour le détail sprint par sprint)
...
**Tests :** 195 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement, `cargo fmt --check` propre.
...
| Interface graphique (`pdf-ui`) | Fenêtre `egui`/`eframe` fonctionnelle : ouverture de fichier (dialogue natif `rfd`), navigation page suivante/précédente, zoom par re-rasterisation (boutons **et** molette+Ctrl/pincement trackpad via `egui::InputState::zoom_delta`), recherche texte plein document **avec surlignage jaune translucide** des occurrences sur la page affichée, **panneau de miniatures** cliquables (`egui::SidePanel`, une par page, rendues à petite échelle et mises en cache), **panneau de signets/plan** arborescent (indentation par profondeur) qui saute à la page au clic, **mode défilement continu** (toggle "📜 Continu") : toutes les pages empilées verticalement dans une `egui::ScrollArea::show_rows` virtualisée, la recherche/les miniatures/les signets/les boutons précédente-suivante y déclenchent un saut de défilement, **sélection de texte à la souris** (glisser sur la page, surlignage bleu translucide, copie via bouton "📋 Copier" ou ⌘C), **et l'édition câblée quasi entièrement** (voir `main.rs`, 2026-07-05) : surligner/souligner/barrer la sélection courante, ajouter du texte `/FreeText` au clic ou remplacer la sélection via une modale de saisie, manipuler les pages (insérer une page blanche/une image, supprimer, pivoter, déplacer par glisser-déposer, fusionner un autre PDF, extraire des pages), "↶ Annuler"/"↷ Rétablir" (activés/désactivés selon `can_undo_edit`/`can_redo_edit`), "💾 Enregistrer" — en plus des raccourcis natifs `⌘Z`/`⌘⇧Z`/`⌘S` (voir le menu natif). Parle à `pdf-app::Session` (état de session : document, page courante, rendu RGBA, recherche, table des matières, sélection, édition) plutôt que directement à `pdf-core`/`pdf-render`. **et remplir un champ de formulaire** (contour bleu cliquable sur chaque champ texte, modale préremplie avec la valeur actuelle, voir §5 point 19). Vérifié visuellement par capture d'écran sur un fixture réel, et via les tests `pdf-app` (surlignage visible au pixel près avant tout `save`) | mode continu : hauteur de ligne uniforme dérivée de la page 0 uniquement (documents à tailles de page hétérogènes mal gérés), sélection de texte limitée au mode page unique (pas en défilement continu), pas de double/triple-clic (mot/ligne) |
| Chrome natif macOS (`pdf-ui/src/native_menu.rs`) | Vraie `NSMenu` système (`objc2`/`objc2-app-kit`), pas dessinée par `egui` : menus Fichier (Ouvrir `⌘O`, **Enregistrer `⌘S`** [nouveau — vrai "Save", écrit dans le fichier ouvert], Exporter une copie `⇧⌘S` [déplacé], Fermer `⌘W`), **Édition** (nouveau — Annuler `⌘Z`, Rétablir `⌘⇧Z`), Affichage (Basculer le mode sombre, Plein écran `⌃⌘F`), Fenêtre (Réduire `⌘M`, Zoomer), Quitter `⌘Q`. Actions standard envoyées le long de la chaîne de répondeurs (cible `nil`) ; actions propres à l'app via `MenuTarget` (`objc2::define_class!`) qui pousse une commande dans un canal MPSC. Glisser-déposer géré nativement par `egui`/`winit`. Mode sombre : bascule réelle de `NSApplication.appearance`. Packaging : `cargo-bundle` + `hdiutil`, signé (`Developer ID Application`, hardened runtime) et **notarisé par Apple** (`.app` et `.dmg` séparément — le `.dmg` doit être signé lui-même pour passer Gatekeeper une fois téléchargé, pas seulement contenir une app signée), vérifié par `spctl`/`hdiutil verify` — voir README.md §Distribution & notarisation | Quick Look non fait (extension d'app séparée, hors périmètre) |
...
- **Signature Developer ID / notarisation Apple** : faites — voir README.md §Distribution & notarisation et §2 ci-dessus.
- **Logique applicative** (`pdf-app`) : porte l'état de session (voir §2) et le rendu inclut les annotations (`run_page_with_annotations`) ; `pdf-app` expose désormais `pdf-edit` (`EditSession` intégrée à `Session`, undo/redo déclenchable depuis `pdf-ui`, voir §2) — seul le remplissage de formulaire n'a pas encore son wrapper, voir la ligne ci-dessous.
- ~~**Remplissage de formulaire au clic**~~ — fait (voir §2, §5 point 19) : `pdf-ui` liste les champs de la page courante (`pdf_app::Session::form_fields_on_current_page`) et ouvre une modale de saisie préremplie au clic sur un champ.
...
28 fixtures PDF dans [pdf-core/tests/fixtures/](./pdf-core/tests/fixtures/) (voir leur [README](./pdf-core/tests/fixtures/README.md) pour le détail et les scripts de régénération — `acroform_checkbox.pdf`/`acroform_radio.pdf`/`acroform_choice.pdf` ajoutés aux Sprints 52-54), plus `sample_image.jpg` (Sprint 15-16 : une petite image JPEG synthétique, pas un PDF, utilisée pour tester `pdf-edit::insert_image_page`) :
...
2. ~~`pdf-app` porte l'état de session~~ — fait (voir §2) ; `pdf-edit` a maintenant son propre historique undo/redo (Sprint 13-14, voir §2), et `pdf-app`/`pdf-ui` l'exposent désormais (`EditSession` intégrée à `Session`, undo/redo déclenchable depuis l'interface, voir §5 point 19).
...
18. ~~Édition de texte 6a/6b (`pdf-edit`)~~ — fait (voir §2, Sprint 17+) : ajout de nouveau texte (`/FreeText`) et remplacement par superposition (masquer + redessiner, sans toucher le flux d'origine) — testés bout en bout. 6c (édition chirurgicale du flux existant) volontairement pas engagé, traité comme projet de recherche séparé.
19. ~~Câblage `pdf-ui` de l'édition~~ — fait, intégralement (2026-07-12) : `pdf-app::Session` porte une `EditSession` interne avec rafraîchissement immédiat du rendu ; `pdf-ui` gagne surlignage/soulignement/barré de la sélection ([main.rs:614-660](./pdf-ui/src/main.rs)), ajout de texte `/FreeText` au clic et remplacement de la sélection via une modale de saisie ([main.rs:1355](./pdf-ui/src/main.rs), [main.rs:691-720](./pdf-ui/src/main.rs)), manipulation de pages complète — insérer/supprimer/pivoter/déplacer par glisser-déposer/fusionner/extraire ([main.rs:759-863](./pdf-ui/src/main.rs)) —, undo/redo, enregistrer (boutons **et** `⌘Z`/`⌘⇧Z`/`⌘S` natifs), et désormais le remplissage de formulaire au clic : `pdf_edit::EditSession::form_fields` (nouveau) liste les champs texte de `/AcroForm/Fields` ; `pdf_app::Session::form_fields_on_current_page`/`set_form_field_value_on_current_page` (nouveaux) filtrent à la page courante et enveloppent `set_form_field_value` avec rafraîchissement ; `pdf-ui` dessine un contour cliquable par champ et ouvre `text_modal` préremplie au clic (`PendingTextAction::FillForm`, [main.rs:1401](./pdf-ui/src/main.rs)). Testé bout en bout côté `pdf-edit`/`pdf-app` sur `acroform_textfield.pdf` ; côté `pdf-ui`, vérifié par build/clippy/lancement sans crash uniquement (pas de test interactif automatisé pour une fenêtre native `egui`). **Avec ce point, toute interaction d'édition exposée par `pdf-edit` a un équivalent `pdf-ui`** — la prochaine valeur significative côté édition est donc au niveau moteur (voir §5 points 14-15, CMaps CJK/Type1) plutôt qu'au niveau interface.

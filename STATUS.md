...
**Dernière mise à jour :** 2026-07-04 (Sprint 15-16 : manipulation de pages — `pdf-edit::EditSession` gagne insertion/suppression/déplacement/rotation de page (arbre `/Pages` aplati paresseusement), fusion/découpage de documents (`copy_object_recursive` + `pdf_core::writer::write_standalone`), insertion d'image JPEG comme page, et un vrai garbage collector par reconstruction (`export_optimized`) — tout testé bout en bout par sauvegarde + réouverture + rendu réel ; voir aussi les mises à jour précédentes : premier socle d'édition (Sprint 13-14), chrome natif macOS (Sprint 11-12), harnais de comparaison pixel + corpus élargi à 25 fixtures (Sprint 0/3-4/7-8) — sprint.md pour le détail sprint par sprint)
...
**Tests :** 189 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement, `cargo fmt --check` propre.
...
17. ~~Manipulation de pages (`pdf-edit`)~~ — fait (voir §2, Sprint 15-16) : insertion/suppression/déplacement/rotation de page, fusion/découpage de documents (copie d'objets récursive avec renumérotation), insertion d'image JPEG comme page, export optimisé (garbage collection par reconstruction) — tout testé bout en bout, `pdf-cli` exerçant chaque opération sur un vrai fichier. **Prochaine valeur significative :** câbler une interface `pdf-ui` pour tout ce qui a été fait aux Sprints 13-14 et 15-16 (annotations, formulaires, pages) — c'est maintenant le principal écart entre "moteur fonctionnel" et "éditeur utile au quotidien" au sens des jalons de ces deux sprints ; les raccourcis ⌘Z/⌘⇧Z sont déjà prévus dans le menu natif (Sprint 11-12) mais pas encore branchés.

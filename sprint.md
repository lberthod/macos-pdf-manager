...
**Critère de sortie (fin Phase 2) :** rendu pixel-comparé conforme sur le corpus, écart sous le seuil défini par le harnais. **Statut réel : quasiment atteint pour le rendu, avec un premier prototype UI en bonus.** Rendu vectoriel, texte (TrueType/CFF intégrés + substitué système + composites `/Type0`/CID) et images (JPEG, canal alpha `/SMask`) sont fonctionnels et validés visuellement (9 fixtures, dont `type0_cid_truetype.pdf`). Il manque : Type1 historique, un fixture `/CIDFontType0` (CFF CID-keyed) réel dans le corpus, un harnais de comparaison pixel automatisé, et un corpus de test large pour véritablement clore cette phase.

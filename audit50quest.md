...
| 50 | VoiceOver + navigation 100 % clavier + PDF chiffrés (mot de passe) | S | ● | ◐ | **Déchiffrement fermé au Sprint 22** pour mot de passe vide, **étendu au vrai mot de passe utilisateur au Sprint 58** : `Decryptor::new` prend désormais le mot de passe candidat et **vérifie** qu'il correspond à `/U` (Algorithme 4/5 ISO 32000-1 §7.6.4.4, ou le sel de validation pour R5/6) avant de renvoyer un contexte utilisable — un mot de passe incorrect renvoie `PdfError::IncorrectPassword` au lieu de produire silencieusement du contenu corrompu (bug réel corrigé, pas seulement une extension). `Document::open_with_password`/`pdf_edit::EditSession::open_with_password`/`pdf_app::Session::open_with_password` exposent le mot de passe de bout en bout ; `pdf-ui` propose une fenêtre de saisie (mot de passe masqué, réessai en boucle) quand l'ouverture avec mot de passe vide échoue. Testé bout en bout sur un nouveau fixture réel (`encrypted_user_password.pdf`, AES-128/R4, mot de passe `"secret123"`) : bon mot de passe déchiffre, mauvais mot de passe et mot de passe vide rejetés proprement. **Correction au Sprint 60 : l'accessibilité n'était pas "totalement absente" comme les sprints précédents l'affirmaient** — `eframe`/`egui` (déjà en usage depuis le Sprint 9-10) embarquent `accesskit`/`accesskit_macos` (pont `NSAccessibility`) en fonctionnalité Cargo **par défaut**, jamais désactivée par `pdf-ui` ; `wgpu_integration.rs::init_accesskit` (le chemin de rendu réellement utilisé, `Renderer::Wgpu`) l'active automatiquement à chaque frame, confirmé par lecture du code source d'`eframe` et la présence d'`accesskit_macos` dans `Cargo.lock`. Le chrome natif (menus, boutons, curseurs, champs de texte) a donc déjà un arbre d'accessibilité fonctionnel, **sans aucun code applicatif à écrire** — une erreur de documentation de longue date, jamais vérifiée par lecture de code avant cette passe, pas une lacune réelle du produit. **Reste réellement absent** : le contenu de page PDF lui-même (rendu comme une simple texture opaque, aucun texte exposé à VoiceOver) — un projet distinct et bien plus large (arbre de texte accessible synchronisé avec le rendu), hors périmètre. **Non vérifié avec un vrai lecteur d'écran** (pas d'accès à une session graphique macOS + VoiceOver actif depuis cet environnement de développement) — validé seulement par lecture de code/dépendances. |

...
**Dernière mise à jour :** 2026-07-04 (Sprint 11-12 : chrome natif macOS — vraie `NSMenu` système via `objc2`/`objc2-app-kit` (`pdf-ui/src/native_menu.rs`), ouverture/export natifs, glisser-déposer, plein écran, mode sombre — et packaging `.app`/`.dmg` via `cargo-bundle`/`hdiutil`, vérifiés ; voir aussi la mise à jour précédente : harnais de comparaison pixel `pdf-render/tests/golden.rs` + `pdf-render-gpu/tests/cross_backend.rs`, corpus élargi de 15 à 25 fixtures, bug de décodage JPEG CMYK corrigé — sprint.md Sprint 0/3-4/7-8/11-12 pour le détail)
...
**Tests :** 169 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement, `cargo fmt --check` propre.

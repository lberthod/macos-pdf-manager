...
**Dernière mise à jour :** 2026-07-04 (Sprint 13-14 : premier socle d'édition — `pdf-core::writer` (sérialisation d'`Object`), `Document::save_incremental` (sauvegarde incrémentale, append xref), `Interpreter::run_page_with_annotations` (rendu des `/AP /N`), et `pdf-edit::EditSession` (annotation `/Highlight`, remplissage de champ de formulaire texte, undo/redo) — tout testé bout en bout par sauvegarde + réouverture + rendu réel, pas seulement par inspection de structure ; voir aussi les mises à jour précédentes : chrome natif macOS (Sprint 11-12), harnais de comparaison pixel + corpus élargi à 25 fixtures (Sprint 0/3-4/7-8) — sprint.md pour le détail sprint par sprint)
...
**Tests :** 182 tests automatisés (`cargo test --workspace`), tous verts, `cargo clippy --workspace --all-targets` sans avertissement, `cargo fmt --check` propre.
...
| Undo/redo (`pdf-edit::EditOp`) | Capture, pour chaque objet **existant** modifié par une opération, sa valeur avant/après ; `undo`/`redo` restaurent l'une ou l'autre. Testé (`undo_and_redo_toggle_annotation_visibility`) | Les objets nouvellement créés par une opération (annotation, apparence, police) restent alloués après un `undo` — rendus non référencés donc invisibles, pas physiquement supprimés (nettoyage = garbage collection des orphelins, Sprint 15-16) ; pas d'interface `pdf-ui` pour déclencher `undo`/`redo` (le menu natif a des raccourcis prévus mais volontairement pas câblés, voir plus haut) |
...
- **Autres types d'annotation, pages, texte** (section 7 d'architecture.md) : notes, formes, texte libre, signatures, manipulation de pages (Sprint 15-16), édition du texte existant (Sprint 17+) — non commencés.
...
25 fixtures dans [pdf-core/tests/fixtures/](./pdf-core/tests/fixtures/) (voir leur [README](./pdf-core/tests/fixtures/README.md) pour le détail et les scripts de régénération) :
...
16. ~~Premier socle d'édition (`pdf-edit`)~~ — fait (voir §2, Sprint 13-14) : sérialisation d'`Object` (`pdf-core::writer`), sauvegarde incrémentale (`Document::save_incremental`), rendu des annotations (`Interpreter::run_page_with_annotations`), et `pdf-edit::EditSession` (annotation `/Highlight`, remplissage de champ de formulaire texte, undo/redo) — tout testé bout en bout (sauvegarde + réouverture + rendu réel). **Prochaine valeur significative :** câbler une interface `pdf-ui` (outil de surlignage à la souris, édition de champ au clic, raccourcis ⌘Z/⌘⇧Z déjà prévus dans le menu natif mais pas encore branchés) — sans ça, l'éditeur n'est pas encore "utile au quotidien" au sens du jalon de Sprint 13-14, seulement fonctionnel au niveau moteur.

...
**Dernière mise à jour :** 2026-07-04 (`pdf-render-gpu` : ajout du trait et de la rotation `/Rotate` — deux dérivations géométriques vérifiées par test avant d'y faire confiance, correctes du premier coup)
...
| Rendu (GPU, `pdf-render-gpu`) | Nouveau crate : pipeline `wgpu` **hors-écran** (`Instance::request_adapter` sans surface), chemins remplis **et tracés** (`lyon::tessellation::{FillTessellator, StrokeTessellator}`) **et** contours de glyphes (même transform que côté CPU), un seul draw call par page, lecture RGBA8 via `copy_texture_to_buffer`. **Rotation `/Rotate` appliquée** (`render_page_rotated`, rotation dans l'espace NDC — même structure que `pdf_render::rotation_matrix` mais sans avoir besoin des dimensions en points, puisque NDC est toujours un carré [-1,1]). L'espace de clip `wgpu` a Y vers le haut comme l'espace page PDF : le mapping page->NDC ne nécessite pas de flip d'axe (contrairement au pixmap CPU) — cette dérivation et celle de la rotation 90° ont chacune été vérifiées par un test dédié avant d'être considérées correctes, et les deux l'étaient dès la première tentative. Validé bout en bout sur un fixture réel (glyphes TrueType tessellés, pas seulement des rectangles synthétiques) | Pas d'images, pas de clip ; pas d'atlas de glyphes (re-tessellation à chaque appel plutôt que mise en cache des contours) ; pas encore branché dans `pdf-ui` (rendu hors-écran seulement, recrée un device `wgpu` à chaque appel — pas adapté à une boucle interactive en l'état) ; `pdf-render` (CPU) reste la référence tant que la parité n'est pas atteinte |
...
14 fixtures dans [pdf-core/tests/fixtures/](./pdf-core/tests/fixtures/) (voir leur [README](./pdf-core/tests/fixtures/README.md) pour le détail et les scripts de régénération) :
...
12. **Back-end GPU `wgpu`** — deux tranches faites (voir §2, `pdf-render-gpu`) : chemins remplis+tracés, glyphes, rotation, tous tessellés/mappés via `lyon`+NDC, rendu hors-écran validé sur fixture réel. Reste à faire pour la parité avec `pdf-render` : images, clip, atlas de glyphes (au lieu de re-tesseller à chaque frame), et surtout le branchement dans `pdf-ui` (device `wgpu` persistant, intégration avec `eframe` qui supporte déjà un backend `wgpu` en interne). C'est le seul point qui garde le Sprint 9-10 ouvert.
11. **Polices composites `/Type0`/CID** — prochaine valeur significative de niveau 2 (grille §2bis) : codes 2 octets, `/DescendantFonts`, `/W` CID. Très répandu en pratique pour le CJK réel (contrairement au fixture de test qui utilise une police simple). Nécessitera un fixture construit à la main (comme `embedded_cff_font.pdf`) : reportlab ne produit pas de `/Type0` embarqué facilement (confirmé en explorant cette piste).

...
| N° | Fonctionnalité | Prio | Moteur | Statut | Preuve |
|---|---|---|---|---|---|
| 43 | Remplissage AcroForm (texte, cases, listes) | S | ● | ◐ | **Champs texte (`/Tx`) fermés** [Sprint 50] : `EditSession::form_fields` + wrappers `pdf-app` + contour cliquable/modale préremplie dans `pdf-ui`. **Cases à cocher (`/Btn`, simples) fermées au Sprint 52** : `EditSession::checkbox_fields`/`set_checkbox_field_value` basculent `/AS`+`/V` sans régénérer l'apparence (`/AP /N` déjà un dictionnaire d'états dans un vrai PDF) ; `pdf-ui` bascule au clic. **Boutons radio groupés (`/Btn`, bit `Radio` + `/Kids`) fermés au Sprint 53** : `EditSession::radio_groups` liste chaque groupe par ses widgets-enfants (le champ parent n'a pas de `/Rect` propre), `set_radio_group_value` fixe l'`/AS` de chaque enfant [`/Off` sauf l'option choisie] et le `/V` du champ en un seul `EditOp` ; `pdf-ui` sélectionne au clic, un contour + remplissage translucide par option. Testé bout en bout sur `acroform_checkbox.pdf`/`acroform_radio.pdf` [fixtures dédiés], y compris une vérification que l'état affecte effectivement le rendu (cases) et qu'un seul widget reste coché après sauvegarde (radio). **Toujours pas de listes/menus déroulants** (`/Ch`) — ligne ◐, pas ☑. |

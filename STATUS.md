...
**Dernière mise à jour :** 2026-07-12 (dernier trou de câblage `pdf-ui` de l'édition comblé : remplir un champ de formulaire au clic. `pdf_edit::EditSession::form_fields` [nouveau] liste les champs texte de `/AcroForm/Fields` [nom/rect/valeur, lus via `current` pour refléter une valeur déjà en attente dans la session] ; `pdf_app::Session::form_fields_on_current_page`/`set_form_field_value_on_current_page` [nouveaux] filtrent aux champs de la page courante — via ses `/Annots`, même mécanique que `annotations_on_current_page` — et enveloppent `set_form_field_value` avec rafraîchissement immédiat du rendu ; `pdf-ui` dessine un contour bleu par champ et ouvre `text_modal` [`PendingTextAction::FillForm`, nouvelle variante] préremplie avec la valeur actuelle au clic, sur le même patron que `handle_add_text_click`/`handle_annotation_click`. Testé bout en bout sur `acroform_textfield.pdf` côté `pdf-edit` et `pdf-app` [valeur vide puis remplie, avant toute sauvegarde] ; côté `pdf-ui`, vérifié par build/clippy/lancement sans crash [pas de vérification interactive en environnement avec affichage depuis cet outil]. **Avec ce point, toutes les interactions d'édition exposées par `pdf-edit` ont désormais un équivalent `pdf-ui`** — voir §2 et §5 point 19 pour le détail de ce qui était déjà câblé au préalable [surlignage/soulignement/barré, `/FreeText` au clic, remplacement de texte, manipulation de pages, undo/redo/enregistrer]. Voir aussi les mises à jour précédentes : premier câblage `pdf-ui` de l'édition [2026-07-05, voir sprint.md "Note de suivi" après le Sprint 17+], édition de texte 6a/6b [Sprint 17+], manipulation de pages [Sprint 15-16], premier socle d'édition [Sprint 13-14], chrome natif macOS [Sprint 11-12], harnais de comparaison pixel + corpus élargi à 25 fixtures [Sprint 0/3-4/7-8] — sprint.md pour le détail sprint par sprint)

...
**Dernière mise à jour :** 2026-07-12 (#43, cases à cocher AcroForm — Sprint 52, voir sprint.md. Nouveau fixture `acroform_checkbox.pdf` [`/FT /Btn`, `/AP /N` dictionnaire d'états `/Off`+`/Yes`, `Ff 2`] : le corpus n'avait qu'un champ texte jusqu'ici. `pdf_edit::EditSession::checkbox_fields` [nouveau] liste les champs `/Btn` qui sont leur propre widget [`/Rect` direct, exclut boutons-poussoir et boutons radio groupés] avec leur `on_state` [nom de l'état "coché" lu dans `/AP /N`, variable d'un générateur à l'autre] ; `set_checkbox_field_value` bascule `/AS`+`/V` **sans régénérer `/AP`** [contrairement au texte : un vrai PDF a déjà les deux apparences, `resolve_normal_appearance` — déjà là depuis le Sprint 13-14 — résout laquelle afficher selon `/AS`]. `pdf_app::Session::checkbox_fields_on_current_page`/`set_checkbox_field_value_on_current_page` enveloppent ; `pdf-ui` dessine un contour [rempli translucide si coché] et bascule l'état au clic, sans modale [contrairement au texte]. Testé bout en bout côté `pdf-edit`/`pdf-app` [coché/décoché, persistance après réouverture, **et vérification que l'état affecte le rendu** — nombre d'éléments de la `DisplayList` différent] ; côté `pdf-ui`, vérifié par build/clippy/lancement sans crash [pas de vérification interactive en environnement avec affichage depuis cet outil]. **Reste ouvert dans #43 : boutons radio groupés et listes/menus déroulants (`/Ch`)**. Juste avant (même session) : #32, poignées de déplacement/redimensionnement d'annotation [Sprint 51] — `pdf_edit::EditSession::set_annotation_rect` [nouveau] ne modifie que `/Rect`+`/QuadPoints` [rescale affine], le flux `/AP /N` existant est mappé dessus sans être régénéré ; `pdf-ui` dessine 4 poignées de coin glissables [`handle_annotation_drag`, un seul `EditOp`/undo par glissement]. **Reste ouvert dans #32 : réglage couleur/opacité après création.** Encore avant : dernier trou de câblage `pdf-ui` de l'édition comblé, remplir un champ de formulaire au clic [Sprint 50] — `pdf_edit::EditSession::form_fields` liste les champs texte de `/AcroForm/Fields`, `pdf_app::Session::form_fields_on_current_page`/`set_form_field_value_on_current_page` les enveloppent, `pdf-ui` dessine un contour bleu par champ et ouvre `text_modal` préremplie au clic — testé bout en bout sur `acroform_textfield.pdf`. Voir aussi les mises à jour précédentes : premier câblage `pdf-ui` de l'édition [2026-07-05, voir sprint.md "Note de suivi" après le Sprint 17+], édition de texte 6a/6b [Sprint 17+], manipulation de pages [Sprint 15-16], premier socle d'édition [Sprint 13-14], chrome natif macOS [Sprint 11-12], harnais de comparaison pixel + corpus élargi à 25 fixtures [Sprint 0/3-4/7-8] — sprint.md pour le détail sprint par sprint)
...
26 fixtures PDF dans [pdf-core/tests/fixtures/](./pdf-core/tests/fixtures/) (voir leur [README](./pdf-core/tests/fixtures/README.md) pour le détail et les scripts de régénération — `acroform_checkbox.pdf` ajouté au Sprint 52), plus `sample_image.jpg` (Sprint 15-16 : une petite image JPEG synthétique, pas un PDF, utilisée pour tester `pdf-edit::insert_image_page`) :

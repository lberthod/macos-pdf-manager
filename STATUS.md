...
**Dernière mise à jour :** 2026-07-12 (Sprint 60 — correction de documentation sur l'accessibilité, voir sprint.md. **`eframe`/`egui` [déjà en usage depuis le Sprint 9-10] embarquent `accesskit` [pont `NSAccessibility`] en fonctionnalité Cargo par défaut, jamais désactivée par `pdf-ui`** : `accesskit_macos` apparaît déjà dans `Cargo.lock`, et `wgpu_integration.rs::init_accesskit` [le chemin de rendu réel sélectionné par `pdf-ui`, `Renderer::Wgpu`] l'active automatiquement à chaque frame, confirmé par lecture du code source d'`eframe` — le chrome natif [menus, boutons, curseurs, champs de texte] a donc déjà un arbre d'accessibilité fonctionnel, **sans code applicatif à écrire**. L'affirmation "aucune trace d'accessibilité dans le code" répétée depuis le Sprint 22 [`audit50quest.md` #50] était une erreur de documentation jamais vérifiée par lecture de code, corrigée ici. **Ce qui reste réellement absent** : le contenu de page PDF lui-même [rendu comme une texture opaque `egui::Image`, aucun texte exposé à VoiceOver] — projet distinct, hors périmètre. **Non vérifié avec un vrai lecteur d'écran** [pas d'accès à une session graphique macOS + VoiceOver actif depuis cet environnement de développement]. #50 reste ◐ dans `audit50quest.md`, mais plus pour la raison précédemment documentée. Juste avant (même session) : Sprint 59 [vérification de signature numérique `/Sig`, module `pdf_core::signature`, PKCS#7/CMS détaché — intégrité cryptographique seulement, pas confiance, testé bout en bout avec un vrai outil tiers `pyhanko`], Sprint 58 [vrai mot de passe utilisateur pour PDF chiffrés, corrige un vrai bug de déchiffrement silencieusement incorrect], Sprint 57 [cache police/contours, `Interpret` ÷5,5 sur 1500 pages], Sprint 56 [fuzzing `cargo-fuzz`, deux débordements arithmétiques réels trouvés et corrigés] — voir sprint.md pour le détail de ces sprints et de tous les précédents (Sprints 49-55 : onglets multi-documents, #32/#43 fermés entièrement, voir `audit50quest.md`).

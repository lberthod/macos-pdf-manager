...
**Dernière mise à jour :** 2026-07-05 (premier câblage `pdf-ui` de l'édition, hors sprint dédié — voir sprint.md "Note de suivi" après le Sprint 17+ : `pdf-app::Session` porte désormais une `pdf_edit::EditSession` interne, qui régénère le `Document` de lecture après chaque édition pour un rendu à jour avant toute sauvegarde ; `pdf-ui` gagne un bouton "🖍 Surligner" (sélection de texte courante), "↶ Annuler"/"↷ Rétablir", "💾 Enregistrer", et le menu natif gagne un vrai `⌘Z`/`⌘⇧Z`/`⌘S` (Sprint 11-12 avait volontairement laissé ces raccourcis non câblés en l'absence d'`EditSession` réelle). Voir aussi les mises à jour précédentes : édition de texte 6a/6b (Sprint 17+), manipulation de pages (Sprint 15-16), premier socle d'édition (Sprint 13-14), chrome natif macOS (Sprint 11-12), harnais de comparaison pixel + corpus élargi à 25 fixtures (Sprint 0/3-4/7-8) — sprint.md pour le détail sprint par sprint)
...
19. ~~Premier câblage `pdf-ui` de l'édition~~ — fait (voir §2, note de suivi après le Sprint 17+ dans sprint.md) : `pdf-app::Session` porte une `EditSession` interne avec rafraîchissement immédiat du rendu ; `pdf-ui` gagne surlignage de sélection, undo/redo, enregistrer (boutons **et** `⌘Z`/`⌘⇧Z`/`⌘S` natifs, désormais réellement câblés). **Prochaine valeur significative :** le reste de l'interface d'édition — ajouter une annotation `/FreeText`, remplir un champ de formulaire au clic, manipuler des pages (insérer/supprimer/déplacer/pivoter/fusionner) depuis `pdf-ui` — puisque seul le surlignage de sélection réutilisait une interaction déjà existante, le reste demande de vraies nouvelles interactions (placement au clic, dialogue de saisie, glisser-déposer de pages).
